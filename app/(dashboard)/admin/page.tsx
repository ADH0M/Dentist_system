    ...
    <div className="flex min-h-screen "> 
      ...
      <div className="p-6 flex-1 mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-card p-6 rounded-xl shadow-sm border border-border">
            <h3 className="text-muted-foreground text-sm font-medium">Admins</h3>
            <p className="text-3xl font-bold mt-2 text-purple-600">{adminUsers}</p>
          </div>
          <div className="bg-card p-6 rounded-xl shadow-sm border border-border">
            <h3 className="text-muted-foreground text-sm font-medium">Customers</h3>
            <p className="text-3xl font-bold mt-2 text-blue-600">{customerUsers}</p>
          </div>
        </div>
                  ...
                  <tr key={user.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      {user.id !== currentUserId && (
                        <>
                          <form action={updateUserRole.bind(null, user.id, user.type === 'admin' ? 'customer' : 'admin')}>
                            <button 
                              type="submit" 
                              className="text-xs px-3 py-1 rounded border border-border hover:bg-accent hover:text-white transition-colors"
                            >
                              {user.type === 'admin' ? 'Make Customer' : 'Make Admin'}
                            </button>
                          </form>
